...
ruv <- RUVg(log1p(fq), hk, k=1, round=FALSE, isLog = TRUE)
counts <- ruv$normalizedCounts
#counts[counts<0] <- 0

# Summarized Experiment
se <- SummarizedExperiment(list(counts=counts),
                           colData=data.frame(batch=batch[1:91],
                                              time_points=bio[1:91]))
...
```{r pca}
pca <- prcomp(t(assay(se)[intersect(de, rownames(counts)),]), center=TRUE, scale=TRUE)
...
```{r pca_cm}
cm <- clusterMany(se, isCount=FALSE, dimReduce="PCA",  nPCADims=c(2, 3, 6), 
...
```{r rsec}
rs <- RSEC(se, isCount=FALSE, nPCADims=c(10, 20), alphas=c(0.2, 0.3),
           mergeCutoff=0.01, clusterFunction="hierarchical01",
           combineProportion=0.5, combineMinSize=3, betas=0.7,
...
# newrs <- plotClusters(rs, whichClusters = clusterLabels(rs)[-1], main="Clusters from RSEC", axisLine=-1, matchToTop=TRUE, resetColors=TRUE)
# plotClusters(newrs, existingColors="all")
```
...
```{r mergeClustersPlot}
mergeClusters(ce, mergeMethod="adjP", plot="adjP", cutoff=0.01, isCount=FALSE)
```

```{r mergeClusters}
ce <- mergeClusters(ce, mergeMethod="adjP", plot="none", cutoff=0.01, isCount=FALSE)
par(mar=plotCMar)
