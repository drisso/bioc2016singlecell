...
```{r pca}
pca <- prcomp(t(assay(se)), center=TRUE)
...
```{r rsec}
rs <- RSEC(se, k0s = 4:15, nPCADims=c(10, 20), alphas=c(0.2, 0.3), 
...
---------------|:-------------:|------:|
nPCADims       |  transform    | nPCADims
alphas         |  clusterD     | alpha
clusterFunction |  clusterD    | clusterFunction    
k0s            |  seqCluster   |  k0 
betas          |  seqCluster   | beta
combineProportion |  combineMany | 
combineMinSize |  combineMany  |
dendroReduce   |  makeDendrogram |
dendroNDims    |  makeDendrogram |
mergeMethod    |  mergeClusters |
mergeCutoff    |  mergeClusters |
