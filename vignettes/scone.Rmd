...
## Additional functionalities of `scone`
...
## for now
load("~/git/scone_analysis/imageAfterFilteringE4.RData")

#----- ExpressionSet
oe <- sf.sc.eSet

#----- Order cells by batch wi bio
bio <- droplevels(oe$MD_expt_condition)
bio <- sub("K5ERP63CKO_UI", "KO", bio)
bio <- sub("K5ERRY_UI", "WT", bio)
oe$MD_expt_condition <- as.factor(bio)

oe <- oe[,order(droplevels(oe$MD_expt_condition),droplevels(oe$MD_c1_run_id))]
rownames(oe) <- sapply(rownames(oe),tolower)
counts <- assayData(oe)$counts_table

## Randomly select samples
sample_filter <- sample(1:NCOL(counts), 100)
head(sample_filter)
...
## Filter genes
gene_filter <- filterCount(counts, nRead = 20, nCell = 50)

oe <- oe[which(gene_filter),]
dim(oe)
counts <- assayData(oe)$counts_table

## Housekeeping genes
hk0 <- scan("~/git/scone_analysis/HKlistD1.txt", what="character")
hk0 <- sapply(hk0,tolower)
names(hk0) <- NULL

hk <- intersect(hk0,rownames(oe))

#----- QC data, rm cols 6:9 NaN
qc <- as.matrix(protocolData(oe)@data[,-(6:9)])
...
se <- SummarizedExperiment(list(counts=counts), colData=DataFrame(qc))

```

# Step 1: Cluster Analysis with `clusterMany`
